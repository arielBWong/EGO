...
class IDTLZ2(DTLZ):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        X_, X_M = x[:, :self.n_obj - 1], x[:, self.n_obj - 1:]
        g = self.g2(X_M)
        out["F"] = self.obj_func(X_, g, alpha=1) * (-1.0)
...
class DTLZ4(DTLZ):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        X_, X_M = x[:, :self.n_obj - 1], x[:, self.n_obj - 1:]
        g = self.g2(X_M)
        out["F"] = self.obj_func(X_, g, alpha=self.alpha) * (-1.0)
...
class DTLZ5(DTLZ):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        out["F"] = self.obj_func(theta, g) * (-1.0)
...
class DTLZ7(DTLZ):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        out["F"] = anp.column_stack([f, (1 + g) * h]) * (-1.0)
